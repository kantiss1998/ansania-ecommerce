...
export const adminCustomerService = {
    async getCustomerStats(userId: number): Promise<CustomerStats> {
        const response = await apiClient.get<ApiResponse<CustomerStats>>(`/admin/customers/${userId}/stats`);
        return response.data.data!;
    },

    async getCustomerActivity(userId: number): Promise<CustomerActivity[]> {
        const response = await apiClient.get<ApiResponse<CustomerActivity[]>>(`/admin/customers/${userId}/activity`);
        return response.data.data || [];
    },
