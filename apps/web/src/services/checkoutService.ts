...
export const checkoutService = {
    async getShippingRates(shippingAddressId: number): Promise<ShippingRate[]> {
        try {
            const response = await apiClient.post<ApiResponse<ShippingRate[]>>('/checkout/shipping', {
    ...
    async createOrder(data: CreateOrderData): Promise<OrderResponse> {
        try {
            const response = await apiClient.post<ApiResponse<OrderResponse>>('/checkout/order', data);
            return response.data.data!;
        } catch (error) {
            throw new Error(getErrorMessage(error));
        }
