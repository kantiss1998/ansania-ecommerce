        ...
        <div className="space-y-8">
            ...
            <div className="grid gap-6 md:grid-cols-3">
                <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-md">
                    ...
                    <h3 className="text-sm font-medium text-gray-500">Total Penjualan (Bulan Ini)</h3>
                ...
                <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-md">
                    ...
                    <div className="mt-4 flex items-center text-sm">
                        <Badge variant="info" className="mr-2">Aktif: {customers?.active_customers || 0}</Badge>
                ...
                <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-md">
                    ...
                    <div className="mt-4 flex items-center text-sm">
                        {inventory && inventory.low_stock_items > 0 && (
                            <Badge variant="warning" className="mr-2">{inventory.low_stock_items} Stok Menipis</Badge>
                        )}
            ...
            <div className="grid gap-8 lg:grid-cols-2">
                ...
                <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                    <div className="border-b border-gray-100 bg-gray-50/50 px-6 py-4 flex items-center justify-between">
                        ...
                        <Link href="/admin/reports/sales" className="text-xs font-medium text-primary-600 hover:underline">
                            Lihat Detail
                        </Link>
                    </div>
                    <div className="p-6">
                        <div className="h-48 w-full flex items-end gap-1">
                            {sales?.performance ? sales.performance.map((item, i) => (
                                <div
                                    key={i}
                                    className="flex-1 bg-primary-100 rounded-t hover:bg-primary-200 transition-all group relative"
                                    style={{ height: `${(item.amount / Math.max(...sales.performance.map(p => p.amount))) * 100}%` }}
                                >
                ...
                <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                    <div className="border-b border-gray-100 bg-gray-50/50 px-6 py-4 flex items-center justify-between">
                        <h3 className="font-semibold text-gray-900">Pertumbuhan Pelanggan</h3>
                        <Link href="/admin/reports/customers" className="text-xs font-medium text-primary-600 hover:underline">
                            Lihat Detail
                        </Link>
                    </div>
                    <div className="p-6">
                        <div className="h-48 w-full flex items-end gap-1">
                            {customers?.growth_performance ? customers.growth_performance.map((item, i) => (
                                <div
                                    key={i}
                                    className="flex-1 bg-secondary-100 rounded-t hover:bg-secondary-200 transition-all group relative"
                                    style={{ height: `${(item.count / Math.max(...customers.growth_performance.map(p => p.count))) * 100}%` }}
                                >
                                    <div className="hidden group-hover:block absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-[10px] px-1.5 py-0.5 rounded z-10 whitespace-nowrap">
                                        {item.count} User
                                    </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {[
                    { label: 'Laporan Produk', href: '/admin/reports/products', count: 'Worst Performers, Best Sellers' },
                    { label: 'Laporan Kategori', href: '/admin/reports/categories', count: 'Revenue by Category' },
                    { label: 'Laporan Voucher', href: '/admin/reports/vouchers', count: 'Usage & ROI' },
                    { label: 'Laporan Inventori', href: '/admin/reports/inventory', count: 'Stock Movement, Valuation' },
                ].map((link) => (
                    <Link
                        key={link.href}
                        href={link.href}
                    ...
                    >
                        <p className="font-medium text-gray-900 group-hover:text-primary-700">{link.label}</p>
...
export default function AdminReportsClient({ initialData }: AdminReportsClientProps) {
    return (
        <Suspense fallback={<div className="p-8 text-center text-gray-500">Mempersiapkan laporan...</div>}>
