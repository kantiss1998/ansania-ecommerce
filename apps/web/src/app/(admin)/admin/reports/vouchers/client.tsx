...
export default function AdminVoucherReportClient() {
    ...
    const totalDiscountGiven = data.reduce((sum, item) => sum + item.total_discount_given, 0);
    const totalRevenueGenerated = data.reduce((sum, item) => sum + item.total_revenue_generated, 0);
        ...
        <div className="space-y-6">
            <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-semibold text-gray-900">Voucher Performance Report</h2>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <p className="mt-1 text-xs text-gray-500">total savings for customers</p>
                </div>
                <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="rounded-lg border border-gray-200 bg-white shadow-sm overflow-hidden">
                <Table>
                    ...
                    <TableBody>
                        {data.length > 0 ? (
                            data.map((voucher) => {
                                const usagePercentage = voucher.usage_limit > 0
                                    ? (voucher.usage_count / voucher.usage_limit) * 100
                                    : 0;
                                const roi = voucher.total_discount_given > 0
                                    ? ((voucher.total_revenue_generated - voucher.total_discount_given) / voucher.total_discount_given) * 100
                                    : 0;
                                ...
                                return (
                                    <TableRow key={voucher.voucher_id}>
                                        ...
                                        <TableCell className="text-success-600">
                                            Rp {voucher.total_revenue_generated.toLocaleString('id-ID')}
                                        </TableCell>
                                        <TableCell>
                                            <span className={`font-semibold ${roi >= 0 ? 'text-success-600' : 'text-error-600'}`}>
                                                {roi >= 0 ? '+' : ''}{roi.toFixed(1)}%
                                            </span>
                                        </TableCell>
                                        <TableCell>
