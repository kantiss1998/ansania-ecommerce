...
export default function AdminYearlySalesClient() {
  ...
  const fetchYearlySales = async () => {
    setIsLoading(true);
    try {
      const response = await apiClient.get<{ data: YearlySalesData[] }>(
        "/admin/reports/sales/yearly",
      );
            ...
            {data.length > 0 ? (
              data.map((row, idx) => {
                ...
                const yoyGrowth = prevYear
                  ? ((row.total_revenue - prevYear.total_revenue) /
                      prevYear.total_revenue) *
                    100
                  : 0;
                ...
                return (
                  <TableRow key={row.year}>
                    ...
                    <TableCell>
                      {row.total_customers.toLocaleString("id-ID")}
                    </TableCell>
                    <TableCell>
                      Rp{" "}
                      {row.total_orders > 0
                    ...
                    <TableCell>
                      {idx > 0 && (
                        <span
                          className={`font-semibold ${yoyGrowth >= 0 ? "text-success-600" : "text-error-600"}`}
                        >
                          {yoyGrowth >= 0 ? "+" : ""}
                          {yoyGrowth.toFixed(1)}%
                        </span>
