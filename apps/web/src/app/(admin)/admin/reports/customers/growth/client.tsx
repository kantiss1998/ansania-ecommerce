...
export default function AdminCustomerGrowthClient() {
  ...
  const totalNewCustomers = data.reduce(
    (sum, item) => sum + item.new_customers,
    0,
  );
  const latestTotal =
    data.length > 0 ? data[data.length - 1].total_customers : 0;
            ...
            {data.length > 0 ? (
              data.map((row, idx) => (
                <TableRow key={idx}>
                  ...
                  <TableCell>
                    <span
                      className={`font-medium ${row.growth_rate >= 0 ? "text-success-600" : "text-error-600"}`}
                    >
                      {row.growth_rate >= 0 ? "+" : ""}
