...
export default function AdminVoucherHistoryClient({
  voucherId,
}: AdminVoucherHistoryClientProps) {
  const [history, setHistory] = useState<VoucherUsage[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [page, setPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);

  const fetchVoucherHistory = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await apiClient.get<{
        data: { items: VoucherUsage[]; total_pages: number };
  ...
  useEffect(() => {
    fetchVoucherHistory();
  }, [voucherId, page, fetchVoucherHistory]);
