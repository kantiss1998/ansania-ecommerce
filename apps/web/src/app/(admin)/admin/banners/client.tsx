                        ...
                        {initialData && initialData.length > 0 ? (
                            initialData.map((banner) => (
                                <TableRow key={banner.id}>
                                    <TableCell>
                                        <div className="h-20 w-36 overflow-hidden rounded border border-gray-200 bg-gray-50">
                                            <img
                                    ...
                                    <TableCell>
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500 truncate max-w-xs">{banner.link_url || 'No Link'}</p>
                                        </div>
                                    </TableCell>
                                    <TableCell>
                            ...
                            <TableRow>
                                <TableCell colSpan={6} className="py-12 text-center text-gray-500">
                                    {initialData ? 'Tidak ada banner ditemukan' : 'Gagal memuat data banner'}
                                </TableCell>
                            </TableRow>
                        )}
                    </TableBody>
...
export default function AdminBannersClient({ initialData }: AdminBannersClientProps) {
    return (
        <Suspense fallback={<div className="p-8 text-center text-gray-500">Memuat banner...</div>}>
