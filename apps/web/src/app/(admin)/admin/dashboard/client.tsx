    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {statCards.map((stat) => (
          <div
            key={stat.title}
            className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition-hover hover:shadow-md"
          >
            <div className="flex items-center justify-between">
              <div className="rounded-full bg-gray-50 p-2">{stat.icon}</div>
              <span
                className={`text-xs font-semibold ${stat.trend === "up" ? "text-success-600" : "text-error-600"}`}
              >
                {stat.change}
              </span>
            </div>
