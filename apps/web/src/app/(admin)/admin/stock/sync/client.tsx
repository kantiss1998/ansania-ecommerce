...
export default function AdminStockSyncClient() {
  ...
  const fetchSyncLogs = async () => {
    ...
    try {
      const response = await apiClient.get<{
        data: { logs: SyncLog[]; last_sync: string };
  ...
  const triggerSync = async () => {
    if (
      !confirm(
        "Sinkronisasi stok dari Odoo? Proses ini mungkin memakan waktu beberapa menit.",
      )
    ) {
      return;
    }
    ...
    <div className="space-y-6">
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between">
          ...
          <Button variant="primary" onClick={triggerSync} isLoading={isSyncing}>
            🔄 Sync Now
          </Button>
        </div>
      </div>
                ...
                return (
                  <TableRow key={log.id}>
                    ...
                    <TableCell>
                      <Badge
                        variant={
                          log.sync_type === "manual" ? "info" : "default"
                        }
                      >
                        {log.sync_type}
                      </Badge>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div className="space-y-2 text-sm text-gray-600">
          ...
          <p>
            • Proses sinkronisasi berjalan di background dan mungkin memakan
            waktu beberapa menit
          </p>
          <p>• Sinkronisasi otomatis berjalan setiap 1 jam sekali</p>
          <p>
            • Anda dapat memicu sinkronisasi manual kapan saja dengan tombol
            &quot;Sync Now&quot;
          </p>
        </div>
      </div>
