...
export default function AdminProductViewsClient() {
  ...
  const fetchProductViews = async () => {
    setIsLoading(true);
    try {
      const response = await apiClient.get<{ data: ProductViewStat[] }>(
  ...
  if (isLoading) {
    return (
      <div className="p-12 text-center text-gray-500">Memuat data views...</div>
    );
  }
                ...
                <TableCell>
                  <div className="h-2 w-24 rounded-full bg-gray-100 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, item.view_count / 10)}%`,
                      }}
                    />
                  </div>
                </TableCell>
