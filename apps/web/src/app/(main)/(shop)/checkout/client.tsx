...
import {
  OrderReview,
  OrderSummaryData,
} from "@/components/features/checkout/OrderReview";
import {
  PaymentMethodSelector,
  PaymentMethod,
} from "@/components/features/checkout/PaymentMethodSelector";
...
function CheckoutContent() {
    ...
    items:
      cart?.items.map((item) => ({
        ...
        variant_info: item.variant
          ? [item.variant.color, item.variant.size, item.variant.finishing]
              .filter(Boolean)
              .join(", ")
          : "",
  ...
  const handleAddressSubmit = async (formData: CreateAddressData) => {
    try {
                ...
                >
                  <div className="relative">
                    {currentStep >= step.number && (
                      <div
                        className={`absolute inset-0 rounded-full bg-gradient-to-r ${step.color} blur-lg opacity-30`}
                      />
                    )}
                    <div
