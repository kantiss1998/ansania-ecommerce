    ...
    (
        ...
        const renderIcon = (icon: LucideIcon | ReactNode, size: string = 'h-5 w-5') => {
            ...
            if (typeof icon === 'function' || (typeof icon === 'object' && icon !== null && 'render' in (icon as any))) {
                ...
                return <IconComponent className={cn(size, 'text-gray-400 transition-colors duration-base', isFocused && 'text-primary')} />;
            }
            return icon;
        };
        ...
        const inputClasses = cn(
            ...
            error ? 'border-destructive/30 bg-destructive/5' : 'border-gray-200 bg-gray-50/50 focus:bg-white',
            leftIcon && 'pl-12',
            rightIcon && 'pr-12'
        );
        ...
        if (floatingLabel && label) {
            return (
                <div className={cn('relative flex flex-col', fullWidth && 'w-full')}>
                    ...
                    {error && <p className="mt-1.5 text-sm text-destructive animate-slide-up">{error}</p>}
                </div>
            );
        }
