...
export function ProductCard({ product }: ProductCardProps) {
    ...
    return (
        <Card hover padding="none" className="overflow-hidden group h-full transition-all duration-base">
            <div className="relative">
                ...
                {isOutOfStock && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-[1px]">
                        <Badge variant="default" className="bg-white/95 text-gray-900 border-none px-4 py-2 font-bold shadow-lg">
                            Stok Habis
                        </Badge>
                    </div>
                )}
                ...
                <Stack spacing={2} className="flex-1">
                    <div className="flex items-center gap-1.5">
                        <div className="flex bg-amber-50 px-2 py-0.5 rounded-lg text-amber-600 items-center gap-1">
                            ...
                            <span className="text-xs font-bold">{product.rating_average.toFixed(1)}</span>
                        </div>
