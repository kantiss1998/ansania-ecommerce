        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <Badge variant={config.variant as any} className="self-start md:self-center">
                        {config.label}
                    </Badge>
                </div>
            </div>
                    ...
                    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                        ...
                        <div className="divide-y divide-gray-100">
                            {order.items?.map((item) => (
                                <div key={item.id} className="flex flex-col gap-4 p-6 sm:flex-row hover:bg-gray-50/30 transition-colors">
                                    <div className="flex-1 space-y-1">
                                        <h3 className="font-bold text-gray-900">{item.product_name}</h3>
                                        <p className="text-sm text-gray-500 font-medium">{item.variant_info}</p>
                                    ...
                                    <div className="flex flex-col items-end gap-3">
                                        ...
                                        {order.status === 'delivered' && (
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                onClick={() => handleOpenReview(item.id, item.product_name)}
                                                className="gap-2"
                                            >
                    ...
                    {order.shipping_address && (
                        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                            ...
                            <div className="space-y-3">
                                <div>
                                    <p className="font-bold text-gray-900">{order.shipping_address.recipient_name}</p>
                                    <p className="text-sm text-gray-500 font-medium">{order.shipping_address.phone}</p>
                                </div>
