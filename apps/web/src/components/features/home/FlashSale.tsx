...
export function FlashSale({ products, endTime }: FlashSaleProps) {
    ...
    return (
        <Section background="dark" className="rounded-3xl overflow-hidden relative" spacing="md">
            ...
            <Stack spacing={8} className="relative z-10">
                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl backdrop-blur-sm border border-white/10">
                        <div className="text-right hidden md:block border-r border-white/10 pr-4 mr-2">
                            <p className="text-xs font-bold uppercase tracking-widest text-destructive">Berakhir Dalam</p>
                        </div>
                ...
                <div className="relative -mx-8 overflow-x-auto px-8 pb-4 scrollbar-hide">
                    <div className="flex gap-6 min-w-max">
                        {products.map((product) => {
                            ...
                            return (
                                <motion.div key={product.id} whileHover={{ y: -5 }} className="w-56 flex-shrink-0">
                                    <Link href={`/products/${product.slug}`}>
                                        <Card padding="none" className="overflow-hidden h-full group">
                                            <div className="relative aspect-square overflow-hidden bg-gray-100">
                                                ...
                                                {discountPercentage > 0 && (
                                                    <Badge variant="error" className="absolute top-2 right-2 shadow-lg scale-110">
                                                        -{discountPercentage}%
                                                    </Badge>
                                                )}
                                            </div>
                                            <CardContent className="p-4">
                                                ...
                                                <div className="flex items-baseline gap-2">
                                                    <span className="font-bold text-primary text-lg">{formatCurrency(product.discount_price || product.base_price)}</span>
                                                    {product.discount_price && (
                                                        <span className="text-[10px] text-gray-400 line-through">{formatCurrency(product.base_price)}</span>
                                                    )}
                                                </div>
                                                ...
                                                <div className="mt-4 space-y-2">
                                                    <div className="flex items-center justify-between text-[10px] font-bold">
                                                        ...
                                                        <span className="text-destructive">80% Terjual</span>
                                                    </div>
                ...
                <div className="md:hidden">
                    <Button href="/products?hasDiscount=true" variant="ghost" fullWidth className="text-white border border-white/20">
                        Lihat Semua Promo
                    </Button>
                </div>
            </Stack>
