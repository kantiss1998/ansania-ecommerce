...
export function FlashSaleForm({ initialData }: FlashSaleFormProps) {
    ...
    const [formData, setFormData] = useState<Partial<FlashSale>>(initialData || {
        name: '',
        description: '',
        start_at: new Date().toISOString().split('T')[0] + 'T00:00',
        end_at: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().split('T')[0] + 'T23:59',
        is_active: false,
        items: []
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const endpoint = isEdit ? `/api/admin/flash-sales/${initialData.id}` : '/api/admin/flash-sales';
    ...
    const handleAddItem = (product: any) => {
        const newItem: Partial<FlashSaleItem> = {
            ...
            discount_price: product.selling_price * 0.8, // Default 20% discount
            stock_limit: 10,
            sold_count: 0
        };
        setFormData({
            ...formData,
            items: [...(formData.items || []), newItem as FlashSaleItem]
        });
    };
    ...
    const updateItem = (index: number, field: keyof FlashSaleItem, value: any) => {
        const newItems = [...(formData.items || [])];
            ...
            <div className="flex items-center justify-between rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                <div>
                    <h2 className="text-xl font-bold text-gray-900">
                        {isEdit ? `Edit Flash Sale: ${initialData.name}` : 'Buat Program Flash Sale'}
                    </h2>
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                <main className="lg:col-span-2 space-y-6">
                    <div className="rounded-xl border border-gray-200 bg-white p-8 shadow-sm space-y-6">
                        <h3 className="font-bold text-gray-900 border-b pb-2">Daftar Produk Flash Sale</h3>
                                        ...
                                        <tr key={idx}>
                                            ...
                                            <td className="px-4 py-4">
                                                <input
                                                    ...
                                                    onChange={(e) => updateItem(idx, 'discount_price', Number(e.target.value))}
                                                />
                                            </td>
                                            <td className="px-4 py-4">
                                                <input
                                                    ...
                                                    onChange={(e) => updateItem(idx, 'stock_limit', Number(e.target.value))}
                                                />
                                            </td>
                                            <td className="px-4 py-4">
                                                <button
                        ...
                        <div className="pt-4 border-t border-gray-100">
                            <Button
                                ...
                                onClick={() => handleAddItem({ id: 1, name: 'Contoh Produk', selling_price: 150000 })}
                            >
                ...
                <aside className="space-y-6">
                    <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
                        <h3 className="font-semibold text-gray-900 border-b pb-2">Jadwal & Status</h3>
                        ...
                        <div className="flex items-center gap-3 pt-4">
                            ...
                            />
                            <label htmlFor="is_active" className="text-sm font-medium text-gray-700">Publikasikan Event</label>
                        </div>
                    </div>
                    ...
                    <div className="rounded-xl border border-warning-200 bg-warning-50 p-6 space-y-2">
                        ...
                        <p className="text-xs text-warning-700 leading-relaxed">
                            Flash sale akan menggantikan harga reguler produk selama periode berlangsung. Pastikan kuota stok cukup untuk menghindari over-selling.
                        </p>
                    </div>
                </aside>
            </div>
