...
describe('API Routes', () => {
    ...
    describe('Auth Routes', () => {
        ...
        it('POST /auth/register should be reachable', async () => {
            ...
            vi.mocked(authService.register).mockResolvedValue({ user: { id: 1, email: 'test@example.com' }, token: 'abc' } as any);
        ...
        it('POST /auth/login should be reachable', async () => {
            const authService = await import('../services/authService');
            vi.mocked(authService.login).mockResolvedValue({ user: { id: 1, email: 'test@example.com' }, token: 'abc' } as any);
    ...
    describe('Product Routes', () => {
        it('GET /products should return products', async () => {
            ...
            vi.mocked(productService.listProducts).mockResolvedValue({ products: [], total: 0, page: 1, limit: 10, totalPages: 0 } as any);
    ...
    describe('Cart Routes', () => {
        ...
        it('GET /cart with header should return cart', async () => {
            ...
            vi.mocked(cartService.getCart).mockResolvedValue({ id: 1, items: [] } as any);
