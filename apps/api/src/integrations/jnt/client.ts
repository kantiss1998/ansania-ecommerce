...
export class JNTClient {
    ...
    private getMockRates(params: JNTRateQuery): JNTRateResponse[] {
        console.log(`[JNT MOCK] Checking rates from ${params.origin_city} to ${params.destination_city} for ${params.weight}kg`);
    ...
    async getProvinces(): Promise<Province[]> {
        ...
        try {
            const response = await fetch(`${this.config.baseUrl}/api/location/provinces`, {
                method: 'GET',
                headers: {
                    'X-API-Key': this.config.apiKey
                }
