...
export class DokuClient {
  ...
  constructor() {
    ...
    this.config = {
      ...
      environment:
        (process.env.DOKU_ENVIRONMENT as "sandbox" | "production") || "sandbox",
      baseUrl: process.env.DOKU_API_URL || "https://api-sandbox.doku.com",
      webhookUrl:
        process.env.DOKU_WEBHOOK_URL ||
        "http://localhost:5000/api/checkout/payment-webhook",
    };
  ...
  async generatePayment(request: PaymentRequest): Promise<PaymentResponse> {
    ...
    try {
      ...
      const response = await fetch(
        `${this.config.baseUrl}/checkout/v1/payment`,
        {
          method: "POST",
          headers: {
      ...
      const data = (await response.json()) as DokuApiResponse;
  ...
  private generateMockPayment(request: PaymentRequest): PaymentResponse {
    ...
    if (request.payment_method_types?.includes("VIRTUAL_ACCOUNT")) {
      mockResponse.va_number = `8888${Math.floor(Math.random() * 1000000000)
        .toString()
        .padStart(10, "0")}`;
    }
  ...
  verifySignature(headers: any, body: any): boolean {
    ...
    if (process.env.SKIP_DOKU_VERIFY === "true") {
      console.warn(
        "[DOKU] Signature verification SKIPPED (SKIP_DOKU_VERIFY=true)",
      );
      return true;
    }
    ...
    try {
      ...
      const timestamp =
        headers["request-timestamp"] || headers["x-doku-timestamp"];
  ...
  async getPaymentStatus(transactionId: string): Promise<any> {
    ...
    try {
      ...
      const response = await fetch(
        `${this.config.baseUrl}/checkout/v1/payment/${transactionId}`,
        {
          method: "GET",
          headers: {
            "Client-Id": this.config.clientId,
