...
export async function getWishlist(
    ...
    include: [
      {
        ...
        include: [
          ...
          { model: ProductRatingsSummary as any, as: "ratingsSummary" }, // eslint-disable-line @typescript-eslint/no-explicit-any
        ],
      },
      {
  ...
  return {
    items: rows.map((row) => {
      const data = row.get({ plain: true }) as Partial<Wishlist> & {
...
export async function checkWishlist(
  ...
  if (variantId)
    (where as Record<string, unknown>).product_variant_id = variantId;
