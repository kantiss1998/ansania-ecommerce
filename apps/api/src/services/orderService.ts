...
export interface OrderListResult {
  data: Order[];
  meta: ReturnType<typeof calculatePagination>;
}
...
export async function createOrder(
  ...
  try {
    ...
    for (const item of cart.items) {
      ...
      if (!variant)
        throw new NotFoundError(`Product Variant for item ${item.id}`);
    ...
    const shippingRates: JNTRateResponse[] = await calculateShipping(
      data.shipping_address_id,
      cart.id,
    );
    const selectedRate = shippingRates.find(
      (r) => r.service === data.shipping_service,
    );
    ...
    const shippingAddress = await Address.findByPk(data.shipping_address_id, {
      transaction,
    });
    ...
    await Shipping.create(
      {
        ...
        address:
          `${shippingAddress.address_line1} ${shippingAddress.address_line2 || ""}`.trim(),
        city: shippingAddress.city,
        province: shippingAddress.province,
        postal_code: shippingAddress.postal_code,
        courier:
          (selectedRate as JNTRateResponse).serviceName ||
...
export async function validateCheckout(
  userId: number,
  data: { shipping_address_id?: number },
...
export async function cancelOrder(
  ...
  if (
    !(
      [ORDER_STATUS.PENDING_PAYMENT, ORDER_STATUS.PROCESSING] as string[]
