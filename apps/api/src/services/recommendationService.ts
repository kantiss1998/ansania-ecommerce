...
import { Product, ProductView, Order, OrderItem, ProductImage, ProductVariant } from '@repo/database';
import { Op, fn, col, literal } from 'sequelize';

/**
 * Get personalized recommendations for a user based on their past behavior
 */
export async function getPersonalizedRecommendations(userId: number, limit: number = 10) {
    ...
    const viewedCategories = await ProductView.findAll({
        ...
        attributes: [
            [fn('COUNT', col('ProductView.id')), 'view_count']
        ],
    ...
    const categoryIds = viewedCategories.map((c: any) => c.product.category_id).filter(Boolean);
    ...
    const purchasedCategoryIds = orders.flatMap(o =>
        (o.items || []).map(item => item.product?.category_id)
    ).filter(Boolean);

    const allRelevantCategoryIds = Array.from(new Set([...categoryIds, ...purchasedCategoryIds]));
 ...
 */
export async function getFrequentlyBoughtTogether(productId: number, limit: number = 5) {
    ...
    const productIds = relatedItems.map((ri: any) => ri.product_id);
