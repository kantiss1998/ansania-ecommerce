...
export async function getCart(
  ...
  const where: Record<string, unknown> = {}; // Using Record for dynamic user_id/session_id assignment
  ...
  if (!cart) {
    ...
    cart = await Cart.create({
      ...
      expires_at: !userId
        ? new Date(
            Date.now() + CART_CONFIG.GUEST_EXPIRY_DAYS * 24 * 60 * 60 * 1000,
          )
        : null, // guest cart expiry
    });
