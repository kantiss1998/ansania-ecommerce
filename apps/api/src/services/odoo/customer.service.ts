...
export class OdooCustomerService {
  ...
  async syncCustomer(userId: number): Promise<number> {
    ...
    try {
      ...
      if (user.odoo_partner_id) {
        console.log(
          `[ODOO_SYNC] User ${user.email} already has Odoo Partner ID: ${user.odoo_partner_id}`,
        );
  ...
  async updateCustomer(userId: number): Promise<void> {
    ...
    try {
      ...
      if (!user.odoo_partner_id) {
        console.warn(
          `[ODOO_SYNC] User ${user.email} not synced to Odoo yet, creating new partner`,
        );
      ...
      if (odooClient.isMockMode()) {
        console.log(
          `[ODOO_SYNC] Mock mode - would update Odoo partner ${user.odoo_partner_id}`,
        );
        return;
      }
      ...
      console.log(
        `[ODOO_SYNC] Customer updated in Odoo - Partner ID: ${user.odoo_partner_id}`,
      );
    } catch (error) {
  ...
  async syncCustomersFromOdoo(
    ...
    try {
      ...
      console.log(
        `📦 Found ${partners.length} partners in Odoo (Limit: ${limit})`,
      );
      ...
      for (const partner of partners) {
        try {
          ...
          console.error(
            `❌ Error syncing partner ${partner.name} (${partner.id}):`,
            err,
          );
          errorsCount++;
        }
  ...
  private async processPartner(partner: OdooPartner): Promise<void> {
    ...
    const userData = {
      ...
      phone:
        partner.mobile || partner.phone
          ? formatPhone((partner.mobile || partner.phone) as string)
          : null,
    ...
    if (user) {
      ...
      user = await User.create({
        ...userData,
        password:
          "$2a$10$PlaceholderHashForOdooSyncedUserButIdeallyShouldBeReset",
  ...
  private async syncAddress(
    ...
    const addressData = {
      ...
      phone:
        partnerData.mobile || partnerData.phone
          ? formatPhone((partnerData.mobile || partnerData.phone) as string)
          : "",
    ...
    if (existingAddress) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await Address.create(addressData as any);
    }
  }
}
