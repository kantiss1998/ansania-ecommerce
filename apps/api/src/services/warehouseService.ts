 ...
 */
export async function findOptimalWarehouse(
  productId: number,
  _latitude?: number,
  _longitude?: number,
) {
  ...
  const bestVariant = variants.sort(
    (a, b) =>
      (a.inventory?.available_quantity || 0) -
      (b.inventory?.available_quantity || 0),
  )[0];

  return bestVariant?.inventory?.odoo_warehouse_id;
}
