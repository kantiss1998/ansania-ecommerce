...
export async function getSalesPerformance(
  ...
  const sales = (await Order.findAll({
    ...
    where: {
      ...
      created_at: {
        [Op.gte]: literal(
          `DATE_SUB(NOW(), INTERVAL ${DASHBOARD_CONFIG.SALES_PERFORMANCE_DAYS} DAY)`,
        ),
      },
    },
    group: ["date"],
...
export async function getInventoryStatus(): Promise<{
  ...
  const lowStockCount = await ProductStock.count({
    where: {
      available_quantity: {
        [Op.gt]: 0,
        [Op.lte]: DASHBOARD_CONFIG.LOW_STOCK_THRESHOLD,
      },
    },
  });
...
export async function getTopProducts(
  limit: number = DASHBOARD_CONFIG.RECENT_LIMIT,
): Promise<OrderItem[]> {
