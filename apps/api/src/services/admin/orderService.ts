...
export async function updatePaymentStatus(
  ...
  await order.update({ payment_status: payment_status as PaymentStatus });
...
export async function updateShippingInfo(
  ...
  if (!order || !order.shipping)
    throw new NotFoundError("Shipping record for this order");
  ...
  if (order.status === ORDER_STATUS.PAID) {
    await order.update({
      status: ORDER_STATUS.SHIPPED as OrderStatus,
      shipped_at: shippingData.shipped_at || new Date(),
    });
  }
...
export async function processRefund(
  ...
  await order.update({
    status: ORDER_STATUS.REFUNDED as OrderStatus,
    payment_status: PAYMENT_STATUS.REFUNDED as PaymentStatus,
