...
export async function processAbandonedCarts() {
  const oneDayAgo = new Date(
    Date.now() - MARKETING_CONFIG.ABANDONED_CART_MIN_HOURS * 60 * 60 * 1000,
  );
  const twoDaysAgo = new Date(
    Date.now() - MARKETING_CONFIG.ABANDONED_CART_MAX_HOURS * 60 * 60 * 1000,
  );
  ...
  for (const cart of abandonedCarts) {
    ...
    const itemsList = (cart.items || [])
      .map(
        (item) =>
          `- ${item.productVariant?.product?.name || "Product"} (x${item.quantity})`,
      )
      .join("\n");
...
export async function sendPromotionalOffers() {
  const thirtyDaysAgo = new Date(
    Date.now() - MARKETING_CONFIG.INACTIVE_USER_DAYS * 24 * 60 * 60 * 1000,
  );
