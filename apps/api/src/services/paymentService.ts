...
export async function handleWebhook(
  ...
  try {
    ...
    if (!order) {
      console.error(
        "[PAYMENT_WEBHOOK] Order not found:",
    ...
    if (existingPayment) {
      console.warn(
        "[PAYMENT_WEBHOOK] Payment already processed for order:",
        order.order_number,
      );
      await transaction.commit();
      return {
    ...
    console.error(
      "[PAYMENT_WEBHOOK] Error processing webhook in service:",
      error,
    );
    throw error;
  }
...
async function handleSuccessfulPayment(
  ...
  console.log(
    "[PAYMENT] Processing successful payment for:",
    order.order_number,
  );
  ...
  await Payment.create(
    {
      order_id: order.id,
      payment_method: payload.payment.method as string,
      payment_provider: PAYMENT_PROVIDER.DOKU,
      transaction_id: payload.transaction.id,
      amount: Number(order.total_amount),
      status: PAYMENT_STATUS.SUCCESS,
      payment_response: JSON.stringify(payload),
      paid_at: new Date(),
    } as unknown as Payment,
    { transaction },
  );
  ...
  await order.update(
    {
      ...
      payment_status: PAYMENT_STATUS.FAILED as PaymentStatus,
    },
    { transaction },
  );
...
async function handleExpiredPayment(
  ...
  await order.update(
    {
      status: ORDER_STATUS.CANCELLED as OrderStatus,
      payment_status: PAYMENT_STATUS.EXPIRED as PaymentStatus,
    },
    { transaction },
  );
