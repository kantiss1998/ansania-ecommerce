...
export async function calculateShipping(
  ...
  console.log(
    `[SHIPPING] Calculating for cart ${cartId} to address ${addressId}`,
  );
  console.log(
    `[SHIPPING] Total weight: ${totalWeightGrams}g (${totalWeightKg}kg)`,
  );
...
async function cacheShippingRates(
  ...
  try {
    ...
    await ShippingCostsCache.bulkCreate(
      cacheEntries as unknown as ShippingCostsCache[],
    );
    console.log(
      `[SHIPPING] Cached ${rates.length} rates until ${expiresAt.toISOString()}`,
    );
  } catch (error) {
