...
export async function getActivityLogsByUser(
  ...
  try {
    ...
    const result = await adminSystemService.listActivityLogsByUser(
      Number(userId),
      req.query,
    );
    res.json({ success: true, data: result });
...
export async function getActivityLogsByEntity(
  ...
  try {
    ...
    const result = await adminSystemService.listActivityLogsByEntity(
      entityType,
      entityId,
      req.query,
    );
...
export async function sendNotification(
  ...
  try {
    const notification = await adminSystemService.createSystemNotification(
      req.body,
    );
