...
export async function getFlashSaleDetail(req: Request, res: Response, next: NextFunction) {
    try {
        const { id } = req.params;
        const flashSale = await adminFlashSaleService.getFlashSaleDetail(Number(id));
        res.json({
            success: true,
            data: flashSale
        });
...
export async function updateFlashSale(req: Request, res: Response, next: NextFunction) {
    try {
        const { id } = req.params;
        const flashSale = await adminFlashSaleService.updateFlashSale(Number(id), req.body);
        res.json({
            success: true,
            data: flashSale
        });
...
export async function addProducts(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        const result = await adminFlashSaleService.addProductsToFlashSale(Number(id), products);
        res.status(201).json({
            success: true,
            data: result
        });
